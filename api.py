...
@app.command("/find-gif")
async def find_gif(ack, body, client):
    await ack()
    command_text = body['text']
    user_id = body['user_id']
    channel_id = body['channel_id']
    print(command_text, user_id, channel_id)
    relevant_gifs = await fetch_gifs(command_text)
    modal_view = create_modal_view(relevant_gifs)
...
@app.action("select_gif")
async def handle_gif_selection(ack, body, say, client):
    await ack()
    value = body["actions"][0]['value']
    value = json.loads(value)
    channel_id = value["channel_id"]
    ...
    try:
        await slack_client.views_update(
            view_id=body["view"]["id"],
            hash=body["view"]["hash"],
            view={"type": "modal", "callback_id": "your_callback_id",
                  "private_metadata": ""},
